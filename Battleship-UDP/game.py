...
class Game:
	...
	def columnExist(self, column):
	    if("A" <= column <= "J"): #is the column A-E?
	        return True
	    else:
	        return False
	...
	def printfield(self, f):
	    ...
	    for v in range(1,len(l)):
	        text += "\n" + spacing.format(v, f['A'][v],f['B'][v],f['C'][v],f['D'][v]) #Adds all the rows with the row number to the left and a spacing of 2
	...
	def placeShips(self, player):
	    ...
	    ### PLAYER INSTRUCTIONS ###
	    print bcolors.OKGREEN + player.name + ", you have 4x4 cells where you can place your ships," +bcolors.ENDC+ "\n"
	    print bcolors.OKGREEN + "Remember not to tell your opponent where you place your ships" +bcolors.ENDC+ " \n"
	    print bcolors.OKGREEN + "Then you say which direction the ship is turned (right, left, up or down)" +bcolors.ENDC+ "\n"

	    print bcolors.PINK + (self.printfield(player.field.field)) + bcolors.ENDC  #prints the player's field
	    ...
	    for x in player.ships:
	        ...
	        while self.columnExist(column) == False or row not in rowlist or cellBusy == True: #loop until the user enters a valid cell
	            userInput = raw_input(bcolors.OKGREEN + player.name + ", in which cell (A-J)(1-10) do you want to place your " + nth[counter] + " ship?" +bcolors.ENDC+ "\n") #user input for cell
	        ...
	        self.clear()
	        print bcolors.PINK + (self.printfield(player.field.field)) + bcolors.ENDC
	        counter += 1

	def newPlayer(self, n, ships, field, bombfield): #Creates a new player with the given ships, field and bombfield
	    newName = raw_input(bcolors.OKBLUE + "Player " + str(n) + ", what's your name?" + bcolors.ENDC+ "\n")
	    while newName == "":
	        newName = raw_input(bcolors.OKGREEN + "Please, enter something" + bcolors.ENDC+ "\n")
	    self.clear()
	...
	def selectCell(self, player): #Lets the player select a cell to bomb
		...
		while self.columnExist(column) == False or row not in rowlist: #loop until given a valid cell
			userInput = raw_input( bcolors.OKBLUE + player.name + ", in which cell (A-D)(1-4) do you want to bomb your enemy?" + bcolors.ENDC +"\n")
